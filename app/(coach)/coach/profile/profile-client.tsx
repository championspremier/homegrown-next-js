...
export default function CoachProfileClient({ coachId, profile, photoUrl }: Props) {
  ...
  const initials = `${firstName.charAt(0)}${lastName.charAt(0)}`.toUpperCase() || "CO";
      ...
      if (existing) {
        ...
        if (avatarFiles.length > 0) {
          await supabase.storage
            .from("profile-photos")
            .remove(avatarFiles.map((f) => `${coachId}/${f.name}`));
        }
      }

      const filePath = `${coachId}/avatar.jpg`;
  ...
  async function handleLogoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const filePath = `${coachId}/logo-${Date.now()}-${i}.jpg`;
      const { error } = await supabase.storage
        .from("profile-photos")
        .upload(filePath, file, { cacheControl: "3600", upsert: true });
      if (error) {
