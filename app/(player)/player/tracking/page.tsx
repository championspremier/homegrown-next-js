...
export default async function PlayerTrackingPage() {
  ...
  const spiderData = {
    ...
    mental: {
      axes: MENTAL_AXES.map((a) => ({ key: a, label: a.split("-").map((w) => w[0].toUpperCase() + w.slice(1)).join(" "), coachOnly: MENTAL_COACH_ONLY.has(a) })),
      scores: capScores(rawScores.mental, MENTAL_COACH_ONLY),
    },
  };
