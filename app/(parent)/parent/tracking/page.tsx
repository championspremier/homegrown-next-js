...
import ParentTrackingClient from "./tracking-client";
...
export default async function ParentTrackingPage({
  ...
  // Fetch linked players with names
  const { data: relationships } = await (supabase as any)
    .from("parent_player_relationships")
    .select("player_id")
    .eq("parent_id", parentId);
  const playerIds: string[] = (relationships || []).map((r: Record<string, unknown>) => r.player_id as string);

  const linkedPlayersArr: { playerId: string; firstName: string }[] = [];
  if (playerIds.length > 0) {
    ...
    for (const p of profiles || []) {
      linkedPlayersArr.push({ playerId: p.id, firstName: p.first_name || "Player" });
    }
  }

  // Resolve player from ?player= query param, fallback to first linked player
  const requestedPlayer = params.player;
  const isLinked = requestedPlayer && playerIds.includes(requestedPlayer);
  const playerId = isLinked ? requestedPlayer : playerIds[0] || parentId;

  const playerName = linkedPlayersArr.find((p) => p.playerId === playerId)?.firstName || "Player";
  ...
  /* ── Points transactions (current quarter) ── */
  ...
  /* ── Curriculum progress (current quarter) ── */
  ...
  for (const t of transactions) {
    ...
    if (st === "Strength & Conditioning") { inc(rawScores.physical, "strength"); inc(rawScores.physical, "conditioning"); }
    if (st === "Speed Training" || st === "HG_SPEED") inc(rawScores.physical, "speed");
    if (st === "Free Nutrition Consultation") inc(rawScores.physical, "conditioning");
  ...
  for (const p of progressRows) {
    ...
    if (cat === "tactical") { const axis = TACTICAL_AXES.includes(period) ? period : currentPeriod; inc(rawScores.tactical, axis); }
    else if (cat === "technical") { const axis = TECHNICAL_AXES.includes(skill) ? skill : "ball-mastery"; inc(rawScores.technical, axis); }
    ...
    } else if (cat === "mental") inc(rawScores.mental, "solo");
  }
  ...
  const pointsData = {
    ...
    history: transactions.map((t) => ({ checked_in_at: t.checked_in_at as string, session_type: t.session_type as string, points: parseFloat(String(t.points || 0)) })),
  };
  ...
  return (
    <ParentTrackingClient
      playerId={playerId}
      playerName={playerName}
      linkedPlayers={linkedPlayersArr}
